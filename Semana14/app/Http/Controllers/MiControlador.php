<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use PhpParser\Node\Expr\FuncCall;

class MiControlador extends Controller
{
    public function index(){
        
        /* Crear tres variables,
        Una que guarde su nombre, 
        otra que guarde su fecha de nacimiento
        y una tercera que sea un arreglo con
        el nombre de sus  comidas favoritas
        */

        $nombre = "Andrea Batres";
        $fechaNacimiento = "2006-09-30";
        $comidasFavoritas = ["Pizza", "Pupusas", "Sandwiches"];

        // 1. La view que queremos retornar
        // 2. La informacion que queremos 
        return view('pagina',compact('nombre'));

    }

    public function catalogo2(){
        $sorbetes = [['id' => 1, 'nombre' => 'fresa', 'precio' => '1.50'],
        ['id' => 2, 'nombre' => 'chocolate', 'precio' => '1.75'],
        ['id' => 3, 'nombre' => 'vainilla', 'precio' => '1.60'],
        ['id' => 4, 'nombre' => 'galleta', 'precio' => '1.80'],
        ['id' => 5, 'nombre' => 'limón', 'precio' => '1.40']];

        return view('sorbetes',compact('sorbetes'));
    }
    
    private $cereales = [['id' => 1, 'nombre' => 'zucaritas', 'precio' => '2.50'],
        ['id' => 2, 'nombre' => 'choco crispis', 'precio' => '2.70'],
        ['id' => 3, 'nombre' => 'corn flakes', 'precio' => '2.30'],
        ['id' => 4, 'nombre' => 'froot loops', 'precio' => '3.00'],
        ['id' => 5, 'nombre' => 'trix', 'precio' => '2.80']
    ];
    
    public function catalogo(){

        $cereales = $this->cereales;
        //Crear otro arreglo asociativo con 5 elementos
        //y tres campos con el id incluido
        return view('cereales',compact('cereales'));

    }

    public function ver($id){
        $cereal = collect($this->cereales)->firstWhere('id',$id);
        return view('cereal',compact('cereal'));
    }
}

